...
const navItems = ['Home', 'About', 'Contact', 'Accounts', 'Services']
    ...
    <header className="border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden gap-2 text-sm font-medium text-slate-600 md:flex">
          {navItems.map((item) => {
            if((!authUser || !authUser.is_admin) && item === "Services") return;
            if((!authUser || authUser.is_admin) && item === "Accounts") return;
